...
impl MenuRenderer {
    pub fn new<'state>(state: &GraphicsState<'state>, menu: &Menu) -> MenuRenderer {
        ...
        tex_names.extend(
            (1..=6)
                .into_iter()
                .map(|i| format!("gfx/menudot{}.lmp", i)),
        );
    ...
    fn cmd_draw_slider(
        ...
        for i in 0..SLIDER_WIDTH {
            self.cmd_draw_glyph(SLIDER_MIDDLE, x + 8 * i, y, scale, glyph_cmds);
        }
    ...
    pub fn generate_commands<'state, 'a>(
        ...
        match *view.body() {
            ...
            MenuBodyView::Dynamic => {
                self.cmd_draw_body_dynamic(state, &active_menu.items(), glyph_cmds)
            }
        }
    }
}
